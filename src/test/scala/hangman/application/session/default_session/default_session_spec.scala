...
class DefaultSessionSpec extends AnyFunSuite:
  test("DefaultSessionSpec.defineGameParams success"):
    ...
    val initializedUserSession =
      userSession.defineGameParams(difficultyNumber, categoryString)

    assert(
      initializedUserSession.getCurrentGameState.getCategory == categoryString
    )
  ...
  test("DefaultSessionSpec.guess success"):
    ...
    assert(
      afterIncorrectGuessSession.getCurrentGameState.getAdt.contains(
        incorrectChar
      )
    )
    assert(
      afterIncorrectGuessSession.getCurrentGameState.getCurentAttemptsCount == 1
    )
