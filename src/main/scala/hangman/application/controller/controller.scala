...
case class GameController(val userSession: Session):
    def processUserInput(difficultyNumber: Int, choosenCategory: String): GameController = 
        val initializedSession = userSession.defineGameParams(difficultyNumber, choosenCategory)
